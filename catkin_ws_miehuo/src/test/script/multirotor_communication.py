...
class Communication:
    ...
    def __init__(self, vehicle_type, vehicle_id):
        ...
        #self.local_pose_sub = rospy.Subscriber(self.vehicle_type+'_'+self.vehicle_id+"/vins_estimator/camera_pose", PoseStamped, self.local_pose_callback,queue_size=1)
        self.cmd_sub = rospy.Subscriber("/xtdrone/"+self.vehicle_type+'_'+self.vehicle_id+"/cmd",String,self.cmd_callback,queue_size=3)

        self.cmd_sub = rospy.Subscriber("/xtdrone/"+self.vehicle_type+'_'+self.vehicle_id+"/planning/pos_cmd",PositionCommand,self.pos_cmd_callback,queue_size=1)
    ...
    def pos_cmd_callback(self, msg):
        ...
        if self.vehicle_id == '0':
            target_raw_pose.position.x = msg.position.x
            target_raw_pose.position.y = msg.position.y
            target_raw_pose.position.z = msg.position.z
        elif self.vehicle_id == '1':
            target_raw_pose.position.x = msg.position.x - 2.5
            target_raw_pose.position.y = msg.position.y 
            target_raw_pose.position.z = msg.position.z
        ...
        target_raw_pose.velocity.x = msg.velocity.x
        target_raw_pose.velocity.y = msg.velocity.y
        target_raw_pose.velocity.z = msg.velocity.z
        
        target_raw_pose.acceleration_or_force.x = msg.acceleration.x
        target_raw_pose.acceleration_or_force.y = msg.acceleration.y
        target_raw_pose.acceleration_or_force.z = msg.acceleration.z

        target_raw_pose.yaw = msg.yaw
        target_raw_pose.yaw_rate = msg.yaw_dot

        target_raw_pose.type_mask = 0
        self.target_motion = target_raw_pose
    ...
    def cmd_pose_enu_callback(self, msg):
        ...
        yaw = self.q2yaw(msg.orientation)
         ...
         # self.target_motion = self.construct_target(x=msg.position.x-4.0,y=msg.position.y-5.0,z=msg.position.z,yaw=yaw)
        #else:
        self.target_motion = self.construct_target(x=msg.position.x,y=msg.position.y,z=msg.position.z,yaw=yaw)
